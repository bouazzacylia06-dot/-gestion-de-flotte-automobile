// feature-flags/index.js ÔÇö Service de Feature Flags (Canary Deployment)
...
/**
 * Canary rollout : d├®termine si un flag est actif pour un userId donn├®.
 * Utilise un hash stable (somme des codes de caract├¿res % 100) pour
 * assurer que le m├¬me utilisateur voit toujours le m├¬me comportement.
 */
function isRolledOut(flag, userId) {
  if (!flag.enabled) return false;
  if (flag.rolloutPercentage >= 100) return true;
  if (flag.rolloutPercentage <= 0)   return false;
  // Hash deterministe bas├® sur userId
...
// ÔöÇÔöÇ ROUTES ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// Retourne tous les flags filtr├®s par r├┤le avec statut canary
...
// PUT /flags/:name ÔÇö Modifier un flag (hot-reload sans red├®marrage)
// En production, cette route devrait ├¬tre prot├®g├®e par JWT admin
app.put('/flags/:name', (req, res) => {
  ...
  writeFlags(data);
  console.log(`­ƒÜ® Flag mis ├á jour: ${req.params.name} ÔåÆ`, update);
...
app.listen(PORT, () => {
  console.log(`­ƒÜ® Feature Flags service d├®marr├® sur :${PORT}`);
